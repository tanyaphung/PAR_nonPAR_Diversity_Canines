...
def parse_args():
	...
	"""
	parser = argparse.ArgumentParser(description="This script computes the proportion of CpG in each window. NOTE that this is intended to compute CpG content in the neutral regions in each window.")
	...
	for each_window, intervals in windows_with_targets.iteritems():
		...
		for each_interval in intervals:
			...
			for each_sequence in sequences[1:]:
				...
				for index in range(1, len(each_sequence)-1):
					if each_sequence[index-1] == 'C' or each_sequence[index+1] == 'G':
						CpG += 1
		if total != 0:
			returnDict[each_window] = [CpG, total, float(CpG)/total]
		else:
			returnDict[each_window] = [CpG, total, 'NA']
